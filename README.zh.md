...
### 🎯 与 Claude Agent SDK API 对齐
可直接替换使用，功能完全对齐——相同的 Agent 循环、工具、会话、权限和 Hooks。现有用户零成本迁移。

### 🔓 开源且易于扩展
MIT 许可证完整源码。轻松通过自定义工具、Provider 和 Hook 进行定制和扩展。

### 🚀 不依赖 Claude Code
纯 TypeScript 实现，独立运行。无需安装或运行 Claude Code —— 可直接对接任何 LLM 供应商使用。

**核心特性：**
- **Agent 循环** —— 观察-思考-行动的自主 Agent 循环
...
| 版本 | 特性 | 状态 |
|---------|----------|--------|
| v0.1.0-alpha | 核心 Agent 循环、14 个工具、3 个供应商、Session、Hooks、权限系统 | ✅ 已发布 |
...
2. **供应商独立** —— 不被单一供应商锁定
3. **轻量核心** —— 专注、易理解的架构
4. **不依赖 Claude Code** —— 纯 TypeScript 实现，独立运行
